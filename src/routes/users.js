...
function check_plugin_cert(cert) {
    ...
    if (cn_parts[0]!="plugin" && cn_parts[0]!="lti" && cn_parts[0]!="worker" && cn_parts[0]!="admin") {
...
/**
 * @api {POST} /users/multiple/id Get the TeSLA IDs for the given user emails. Using RFC4122 v4
 * @apiName GetUsers
 * @apiGroup Users
 *
 * @apiParam {Array} mails Array of user emails.
 *
 * @apiSuccess {Array} tesla_id email for the user emails requests. If email does not exist, in this case do not return anything
 *
 * @apiExample {curl} Example usage:
 *     curl -X POST --url https://tip.test.tesla-project.eu/users/multiple/id -H 'content-type: application/json' -d '{"mails": ["student_test_1@uoc.edu", "student_test_2@uoc.edu"]}'
 * @apiSuccessExample {json} Success-Response:
 *     HTTP/1.1 200 OK
 *     {
 *      "student_test_1@uoc.edu":"f5f16359-9c41-49ff-b28e-265cc9a69ed3",
 *      "student_test_2@uoc.edu":"516b9638-9c14-4b08-90ee-cb2eb33cd981"
 *     }
 */
router.post('/multiple/id', use_auth, function(req, res, next) {
    var mails = req.body.mails;

    models.getTeslaIDsByEmails(mails, function(data) {
        var ret_data = {};

        for (var i=0; i < data.length; i++) {
            ret_data[data[i].email] = data[i].tesla_id;
        }
        res.send(ret_data);
    });
});
